...
#SBATCH -J garisom_optim_imput-leaft_ccr_leaftemp
#SBATCH -c 32
#SBATCH --mem=128G
...
EXP="leaftemp"
EXP_DIR="output/ccr-imput-leaftemp"
EXP_OUT="$EXP_DIR/${EXP}/optim.out"

mkdir -p "${EXP_DIR}/${EXP}/"

echo "$(date '+%Y-%m-%d %H:%M:%S') - EXPERIMENT ${EXP} POPULATION CCR" >> $EXP_OUT
python run_param_optim.py -i run_configs/ccr/${EXP}.json -o $EXP_DIR -m ../garisom/02_program_code -pd ../DBG-ground-leaftemp/ 2>&1 >> $EXP_OUT
