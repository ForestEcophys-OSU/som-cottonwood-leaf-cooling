...
#SBATCH -J garisom_optim_nrv_pressures
#SBATCH -c 32
#SBATCH --mem=128G
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - EXPERIMENT ${EXP} POPULATION NRV" >> $EXP_OUT
python run_param_optim.py -i run_configs/nrv/${EXP}.json -o $EXP_DIR -m ../garisom/02_program_code 2>&1 >> $EXP_OUT
